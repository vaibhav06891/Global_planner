...
classdef Freenect < handle
    ...
    properties (Constant, Hidden)
        ...
        SYNC_HEADER  = fullfile(Freenect.INSTALLED_PATH,'include/libfreenect/libfreenect_sync.h');
    ...
    methods
        function OBJ = Freenect( CAM_ID )
            fprintf(1, 'Freenect::Freenect(): Enter\n');
            ...
            if (isfield(gCameraMap,sprintf('CAM%d',CAM_ID)))
                fprintf(1, 'Freenect::Freenect(): CAM%d is field\n', CAM_ID);
                throw(MException('Freenect:cameraTaken',...
                    sprintf('Camera ID %d Already in Use!',CAM_ID)));
            else
                fprintf(1, 'Freenect::Freenect(): CAM%d is NOT field\n', CAM_ID);
            ...
            fprintf(1, 'Freenect::Freenect(): Exit\n');
        end
        ...
        % Command Kinect tilt joint to desired pose
        function setTiltDeg(OBJ, a_angle)
            fprintf(1, 'Freenect::setTiltDeg(): Enter\n');
            
            % Enforce Kinect tilt mechanical limits
            if ( abs(a_angle) > 28 )
                fprintf(1, 'Freenect::setTiltDeg(): ERROR - commanded angle out of mechanical range of Kinect!\n');
                return;
            end
            
            calllib('freenectAlias','freenect_sync_set_tilt_degs',...
                int32(a_angle), OBJ.internalCameraId);
            fprintf(1, 'Freenect::setTiltDeg(): Exit\n');
        end
    end
    ...
    methods (Static,Access=protected)
        function [] = ensureInitialized()
            fprintf(1, 'Freenect::ensureInitialized(): Enter\n');
            persistent gAlreadyInitialized;
            
            if ( isempty(gAlreadyInitialized) )
                fprintf(1, 'Freenect::ensureInitialized(): gAlreadyInitialized empty.\n');
                try
                ...
                OTHER_HEADERS = fullfile(Freenect.INSTALLED_PATH,'include/libfreenect');
                ...
                fprintf(1, 'Freenect::ensureInitialized(): Loading library\n');
            ...
            fprintf(1, 'Freenect::ensureInitialized(): Exit\n');
        end
